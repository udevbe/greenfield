...
class AppEndpointSession {
  ...
  handleConnection (webSocket, query) {
    ...
    if (query.launch) {
      ...
      if (appConfig) {
        // TODO do some error checking and report back when the app failed to start
        try {
          const childProcess = child_process.spawn(appConfig.bin, appConfig.args)
          this._nativeCompositorSession.childSpawned(childProcess, webSocket)
        } catch (e) {
          // TODO report to browser that app failed to start
        }
      } else {
