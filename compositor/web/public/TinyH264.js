...
if (ENVIRONMENT_IS_NODE) {
  ...
  if (!Module['thisProgram']) {if (process['argv'].length > 1) {Module['thisProgram'] = process['argv'][1].replace(/\\/g, '/')} else {Module['thisProgram'] = 'unknown-program'}}
  ...
  if (typeof read != 'undefined') {Module['read'] = function shell_read (f) {return read(f)}} else {Module['read'] = function shell_read () {throw'no read() available'}}
...
function integrateWasmJS () {
  ...
  if (typeof Module['locateFile'] === 'function') {
    if (!isDataURI(wasmTextFile)) {wasmTextFile = Module['locateFile'](wasmTextFile)}
    if (!isDataURI(wasmBinaryFile)) {wasmBinaryFile = Module['locateFile'](wasmBinaryFile)}
    if (!isDataURI(asmjsCodeFile)) {asmjsCodeFile = Module['locateFile'](asmjsCodeFile)}
  }
  var wasmPageSize = 64 * 1024
  var info = {
    'global': null,
    'env': null,
    'asm2wasm': { 'f64-rem': (function (x, y) {return x % y}), 'debugger': (function () {debugger}) },
    'parent': Module
  }
  var exports = null
  ...
  function getBinary () {
    try {
      ...
      if (Module['readBinary']) {return Module['readBinary'](wasmBinaryFile)} else {throw'on the web, we need the wasm binary to be preloaded and set on Module[\'wasmBinary\']. emcc.py will do that for you when generating HTML (but not JS)'}
    } catch (err) {abort(err)}
  }
  ...
  var wasmReallocBuffer = (function (size) {
    ...
    if (Module['usingWasm']) {
      try {
        ...
        if (result !== (-1 | 0)) {return Module['buffer'] = Module['wasmMemory'].buffer} else {return null}
      } catch (e) {return null}
    }
  })
  ...
  Module['asm'] = (function (global, env, providedBuffer) {
    ...
    if (!env['table']) {
      ...
      } else {env['table'] = new Array(TABLE_SIZE)}
...
var _h264bsdAlloc = Module['_h264bsdAlloc'] = (function () {return Module['asm']['_h264bsdAlloc'].apply(null, arguments)})
var _h264bsdDecode = Module['_h264bsdDecode'] = (function () {return Module['asm']['_h264bsdDecode'].apply(null, arguments)})
var _h264bsdFree = Module['_h264bsdFree'] = (function () {return Module['asm']['_h264bsdFree'].apply(null, arguments)})
var _h264bsdInit = Module['_h264bsdInit'] = (function () {return Module['asm']['_h264bsdInit'].apply(null, arguments)})
var _h264bsdShutdown = Module['_h264bsdShutdown'] = (function () {return Module['asm']['_h264bsdShutdown'].apply(null, arguments)})
