...
export const createX264OpaqueEncoder: FrameEncoderFactory = (
  width: number,
  height: number,
  wlShmFormat: SupportedWlShmFormat,
): FrameEncoder => new X264OpaqueEncoder(width, height, wlShmFormat)

export class X264OpaqueEncoder implements FrameEncoder {
  ...
  private async _encodeFragment(
    ...
    if (this._opaque === undefined) {
      throw new Error('BUG. Expected an opaque buffer.')
    }
