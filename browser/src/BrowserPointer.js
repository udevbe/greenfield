...
export default class BrowserPointer extends BrowserSurfaceRole {
  ...
  _handleMouseUp (event) {
    ...
    if (this.focus && this.focus.browserSurface) {
      ...
      if (this.grab || nroPopups) {
        const surfaceResource = this.focus.browserSurface.resource
        this._doPointerEventFor(surfaceResource, (pointerResource) => {
          pointerResource.button(this.browserSeat.nextButtonSerial(false), event.timeStamp, linuxInput[event.button], released)
          if (pointerResource.version >= 5) {
            pointerResource.frame()
          }
        })
  ...
  _handleMouseDown (event) {
    ...
    if (this.focus && this.focus.browserSurface) {
      ...
      const surfaceResource = this.focus.browserSurface.resource
      this._doPointerEventFor(surfaceResource, (pointerResource) => {
        pointerResource.button(this.browserSeat.nextButtonSerial(true), event.timeStamp, linuxInput[event.button], pressed)
        if (pointerResource.version >= 5) {
          pointerResource.frame()
        }
      })
