...
struct wlr_linux_dmabuf_v1 {
    ...
    struct drm_format_set default_formats; // for legacy clients
...
static bool check_import_dmabuf(struct wlr_linux_dmabuf_v1 *linux_dmabuf,
    ...
    for (int i = 0; i < attribs->n_planes; i++) {
        ...
        if (drmPrimeFDToHandle(linux_dmabuf->main_device_fd, attribs->fd[i], &handle) != 0) {
            wfl_log_errno(stderr, "Failed to import DMA-BUF FD");
            return false;
        }
        if (drmCloseBufferHandle(linux_dmabuf->main_device_fd, handle) != 0) {
            wfl_log_errno(stderr, "Failed to close buffer handle");
            return false;
        }
    }
    return true;
...
static void params_create_common(struct wl_resource *params_resource,
    ...
    if (flags != 0) {
        wfl_log(stderr, "dmabuf flags aren't supported");
        goto err_failed;
    }
...
static ssize_t get_drm_format_set_index(const struct drm_format_set *set,
...
static struct wlr_linux_dmabuf_feedback_v1_compiled *feedback_compile(
    ...
    if (!allocate_shm_file_pair(table_size, &rw_fd, &ro_fd)) {
        wfl_log(stderr, "Failed to allocate shm file for format table");
        return NULL;
    }
    ...
    for (size_t i = 0; i < feedback->tranches_len - 1; i++) {
        ...
        if (!wl_array_add(&compiled_tranche->indices, table_len * sizeof(uint16_t))) {
            wfl_log(stderr, "Failed to allocate tranche indices array");
            goto error_compiled;
        }
        ...
        for (size_t j = 0; j < tranche->formats->len; j++) {
            ...
            for (size_t k = 0; k < fmt->len; k++) {
                ...
                if (index < 0) {
                    wfl_log(stderr, "Format 0x%" PRIX32 " and modifier "
    ...
    if (!wl_array_add(&fallback_compiled_tranche->indices,
        ...
        wfl_log(stderr, "Failed to allocate fallback tranche indices array");
        goto error_compiled;
    }
...
static void surface_addon_destroy(struct addon *addon) {
...
static struct wlr_linux_dmabuf_v1_surface *surface_get_or_create(
        struct wlr_linux_dmabuf_v1 *linux_dmabuf,
        struct westfield_surface *wlr_surface) {
    struct addon *addon =
            addon_find(&wlr_surface->addons, linux_dmabuf, &surface_addon_impl);
    if (addon != NULL) {
        struct wlr_linux_dmabuf_v1_surface *surface =
                wl_container_of(addon, surface, addon);
        return surface;
    ...
    addon_init(&surface->addon, &wlr_surface->addons, linux_dmabuf,
                   &surface_addon_impl);
...
static void linux_dmabuf_send_formats(struct wlr_linux_dmabuf_v1 *linux_dmabuf,
    ...
    for (size_t i = 0; i < linux_dmabuf->default_formats.len; i++) {
        const struct drm_format *fmt = linux_dmabuf->default_formats.formats[i];
...
static void linux_dmabuf_v1_destroy(struct wlr_linux_dmabuf_v1 *linux_dmabuf) {
    wl_signal_emit(&linux_dmabuf->events.destroy, linux_dmabuf);
    ...
    drm_format_set_finish(&linux_dmabuf->default_formats);
...
static bool set_default_feedback(struct wlr_linux_dmabuf_v1 *linux_dmabuf,
    ...
    if (drmGetDeviceFromDevId(feedback->main_device, 0, &device) != 0) {
        wfl_log_errno(stderr, "drmGetDeviceFromDevId failed");
        goto error_compiled;
    }
    ...
    if (device->available_nodes & (1 << DRM_NODE_RENDER)) {
        ...
        wfl_log(stderr, "DRM device %s has no render node, "
    ...
    if (main_device_fd < 0) {
        wfl_log_errno(stderr, "Failed to open DRM device %s", name);
        goto error_compiled;
    }

    struct drm_format_set formats = {0};
    for (size_t i = 0; i < feedback->tranches_len; i++) {
        const struct wlr_linux_dmabuf_feedback_v1_tranche *tranche = &feedback->tranches[i];
        for (size_t j = 0; j < tranche->formats->len; j++) {
            const struct drm_format *fmt = tranche->formats->formats[j];
            for (size_t k = 0; k < fmt->len; k++) {
                if (!drm_format_set_add(&formats, fmt->format, fmt->modifiers[k])) {
                    goto error_formats;
                }
            }
        }
...
error_formats:
    drm_format_set_finish(&formats);
error_compiled:
    compiled_feedback_destroy(compiled);
    return false;
}

static struct wlr_linux_dmabuf_v1 *wlr_linux_dmabuf_v1_create(struct wl_display *display,
                                                       int version, const struct wlr_linux_dmabuf_feedback_v1 *default_feedback) {
