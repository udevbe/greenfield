...
async function main() {
  ...
  const remoteGtk3URLButton: HTMLButtonElement = document.createElement(
    "button"
  );
  remoteGtk3URLButton.textContent = "GTK3-Demo";
  const remoteKwriteURLButton: HTMLButtonElement = document.createElement(
    "button"
  );
  ...
  remoteGtk3URLButton.onclick = () =>
    (urlInput.value = `docker:udevbe/gtk3-demo:latest`);
  remoteKwriteURLButton.onclick = () =>
    (urlInput.value = `docker:udevbe/kwrite:latest`);
  ...
  launchButton.onclick = async () => {
    ...
    if (applicationLaunchResponse.status === 201) {
      const compositorProxyURL = applicationLaunchResponse.headers.get(
        "location"
      );
      if (compositorProxyURL) {
        await compositorProxyConnector.connectTo(
          new URL(compositorProxyURL),
          JWT_TOKEN
        );
      }
    }
