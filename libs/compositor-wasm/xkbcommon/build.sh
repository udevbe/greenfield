...
build() {
    ...
    pushd repo-xkeyboard-config
      pipx run meson setup --wipe build/ --cross-file "${_SDK_DIR}/emscripten-toolchain.ini" --cross-file "${_SDK_DIR}/emscripten-build.ini" -Dprefix="${_SDK_DIR}/xkbcommon/repo/" -Dxkb-base="/usr/share"
